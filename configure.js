...
try {
ARG_ENABLE("appdynamics_agent", "whether to enable AppDynamics agent support", "yes");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_ENABLE("appdynamics_asserts", "whether to enable asserts in AppDynamics agent", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_WITH("apr", "for apr location", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_WITH("apr-util", "for apr-util location", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_WITH("expat", "for expat location", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_WITH("curl8", "for curl8 location", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_WITH("libiconv", "for libiconv location", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_WITH("log4cxx", "for log4cxx location", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_WITH("zeromq", "for zeromq location", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_WITH("protobuf", "for protobuf location", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_WITH("boost", "for boost location", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_WITH("mysql", "for mysql location", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_WITH("libxml2", "for libxml location", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_WITH("pthread", "for pthread location", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_WITH("php-src", "for php src location", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
ARG_ENABLE('appdynamics-asserts', 'Enable asserts in the AppDynamics agent', 'yes');
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
try {
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\TSRM\\config.w32'));
// vim:ft=javascript

ADD_SOURCES("TSRM", "TSRM.c tsrm_win32.c");
ADD_FLAG("CFLAGS_BD_TSRM", "/D ZEND_ENABLE_STATIC_TSRMLS_CACHE=1");
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\sapi\\apache2handler\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\sapi\\cgi\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\sapi\\cli\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\sapi\\embed\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\sapi\\phpdbg\\config.w32'));
/* ARG_ENABLE('phpdbg', 'Build phpdbg', 'no'); */
/* ARG_ENABLE('phpdbgs', 'Build phpdbg shared', 'no'); */
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\appdynamics_agent\\config.w32'));
/* ARG_ENABLE("appdynamics_agent", "whether to enable AppDynamics agent support", "yes"); */
/* ARG_ENABLE("appdynamics_asserts", "whether to enable asserts in AppDynamics agent", "no"); */
/* ARG_WITH("apr", "for apr location", "no"); */
/* ARG_WITH("apr-util", "for apr-util location", "no"); */
...
if (PHP_APPDYNAMICS_AGENT != "no") {
		
		// Substitute APPDYNAMICS_AGENT_SHARED_LIBADD
		ADD_FLAG('APPDYNAMICS_AGENT_SHARED_LIBADD', '');
		CHECK_LIB('stdc++', null, 'APPDYNAMICS_AGENT_SHARED_LIBADD', true);

		// Add pthread library
		if(PHP_PTHREAD == "no") {
			ERROR('This extension requires pthread. Use --with-pthread=<PATH> to specify the prefix where pthread headers and library are located.');
		}

		PTHREAD_INCLUDE_PATH = PHP_PTHREAD + "\\include";
        PTHREAD_LIB_PATH = PHP_PTHREAD + "\\lib";

		ADD_FLAG("CFLAGS", '/I"' + PTHREAD_INCLUDE_PATH + '"');
		ADD_FLAG("LDFLAGS", '/LIBPATH:"' + PTHREAD_LIB_PATH + '"');
		ADD_FLAG("LIBS", "pthreadVC3.lib");
		ADD_FLAG("CFLAGS", '/DHAVE_STRUCT_TIMESPEC');


		/* ARG_ENABLE('appdynamics-asserts', 'Enable asserts in the AppDynamics agent', 'yes'); */
        if (PHP_APPDYNAMICS_ASSERTS != "no") {
		    AC_DEFINE('BOOST_ENABLE_ASSERT_HANDLER', 1, 'Enable asserts in the AppDynamics agent.');
		} else {
		    AC_DEFINE('BOOST_DISABLE_ASSERTS', 1, 'Disable asserts in the AppDynamics agent.');
		}

        
        // Add log4cxx
		if (PHP_LOG4CXX == "no") {
		    ERROR('This extension requires log4cxx. Use --with-log4cxx=<PATH> to specify the prefix where log4cxx headers and library are located.');
		}
        
		LOG4CXX_INCLUDE_PATH = PHP_LOG4CXX + "\\include"
		LOG4CXX_LIB_PATH = PHP_LOG4CXX + "\\lib"

        ADD_FLAG('CFLAGS', '/I"' + LOG4CXX_INCLUDE_PATH + '"');
		ADD_FLAG('LDFLAGS', '/LIBPATH:"' + LOG4CXX_LIB_PATH + '"');
		ADD_FLAG('LIBS', 'log4cxx.lib');

        // Add APR
		if(PHP_APR == "no") {
			ERROR('This extension requires apr. Use --with-apr=<PATH> to specify the prefix where apr library is located.');
		}

		APR_LIB_PATH = PHP_APR + '\\lib'
        ADD_FLAG('LDFLAGS', '/LIBPATH:"' + APR_LIB_PATH + '"');
		ADD_FLAG('LIBS', 'libapr-1.lib');
        
		// Add APR-UTIL
		if(PHP_APR_UTIL == "no") {
			ERROR('This extension requires apr-util. Use --with-apr-util=<PATH> to specify the prefix where apr-util library is located.');
		}

		APR_UTIL_LIB_PATH = PHP_APR_UTIL + '\\lib'
        ADD_FLAG('LDFLAGS', '/LIBPATH:"' + APR_UTIL_LIB_PATH + '"');
		ADD_FLAG('LIBS', 'libaprutil-1.lib');

        // Add Expat
		if(PHP_EXPAT == "no") {
			ERROR('This extension requires expat. Use --with-expat=<PATH> to specify the prefix where expat library is located.');
		}

		EXPAT_LIB_PATH = PHP_EXPAT + '\\lib'
        ADD_FLAG('LDFLAGS', '/LIBPATH:"' + EXPAT_LIB_PATH + '"');
		ADD_FLAG('LIBS', 'libexpat.dll.a');

        // Add ZeroMQ
        if (PHP_ZEROMQ == "no") {
		    ERROR('This extension requires zeromq. Use --with-zeromq=<PATH> to specify the prefix where zeromq headers and library are located.');
		}

		ZEROMQ_INCLUDE_PATH = PHP_ZEROMQ + "\\include"
		ZEROMQ_LIB_PATH = PHP_ZEROMQ + "\\lib"
	    ADD_FLAG('CFLAGS', '/I"' + ZEROMQ_INCLUDE_PATH + '"');
        ADD_FLAG('LDFLAGS', '/LIBPATH:"' + ZEROMQ_LIB_PATH + '"');
		ADD_FLAG('LIBS', 'libzmq-v142-mt-4_3_4.lib');

        // Add Protobuf
        if (PHP_PROTOBUF == "no") {
		    ERROR('This extension requires protobuf. Use --with-protobuf=<PATH> to specify the prefix where protobuf headers and library are located.');
		}
		
		PROTOBUF_INCLUDE_PATH = PHP_PROTOBUF + "\\include"
		PROTOBUF_LIB_PATH = PHP_PROTOBUF + "\\lib"
		ADD_FLAG('CFLAGS', '/I"' + PROTOBUF_INCLUDE_PATH + '"');
		ADD_FLAG('LDFLAGS', '/LIBPATH:"'+ PROTOBUF_LIB_PATH + '"');
		ADD_FLAG('LIBS', 'libprotobuf.lib');
		ADD_FLAG('LIBS', 'libprotoc.lib');

        // Add Boost
		if(PHP_BOOST == "no") {
			ERROR('This extension requires boost. Use --with-boost=<PATH> to specify the prefix where boost headers and library are located.');
		}

		BOOST_INCLUDE_PATH = PHP_BOOST + "\\include"
		BOOST_LIB_PATH = PHP_BOOST + "\\lib"
		ADD_FLAG('CFLAGS', '/I"' + BOOST_INCLUDE_PATH + '"');
		ADD_FLAG('LDFLAGS', '/LIBPATH:"'+ BOOST_LIB_PATH + '"');
		ADD_FLAG('LIBS', 'libboost_system-vc142-mt-x64-1_75.lib');
		ADD_FLAG('LIBS', 'libboost_filesystem-vc142-mt-x64-1_75.lib');
		ADD_FLAG('LIBS', 'libboost_regex-vc142-mt-x64-1_75.lib');
	    
        // Add MySQL 
		if(PHP_MYSQL == "no") {
			ERROR('This extension requires mysql. Use --with-mysql=<PATH> to specify the prefix where mysql headers and library are located.');
		}

		MYSQL_INCLUDE_PATH = PHP_MYSQL + "\\include"
		ADD_FLAG('CFLAGS', '/I"' + MYSQL_INCLUDE_PATH + '"');

        // Add libxml2
		if(PHP_LIBXML2 == "no") {
			ERROR('This extension requires libxml. Use --with-libxml2=<PATH> to specify the prefix where libxml headers and library are located.');
		}

		LIBXML_INCLUDE_PATH = PHP_LIBXML2 + "\\include\\libxml2"
		LIBXML_LIB_PATH = PHP_LIBXML2 + "\\lib"
		ADD_FLAG('CFLAGS', '/I"' + LIBXML_INCLUDE_PATH + '"');
		ADD_FLAG('LDFLAGS', '/LIBPATH:"'+ LIBXML_LIB_PATH + '"');
		ADD_FLAG("LIBS", "libxml2.lib");

		ADD_FLAG("CFLAGS", "/FS");
    	ADD_FLAG("CXXFLAGS", "/FS");

		ADD_FLAG('CFLAGS', '/EHsc');
		ADD_FLAG("CXXFLAGS", "/EHsc");

		ADD_FLAG("CFLAGS", '/DPROTOBUF_USE_DLLS');

		// Define CXXFLAGS for the Windows environment
		APPD_CXXFLAGS="-include config.h \
		               -Werror \
		               -Wextra \
		               -Wall \
		               ...
		               -fstack-protector \
		               -D_FORTIFY_SOURCE=2 \
		               -DTIMER_USE_CGT_THREAD \
		               -DZEND_ENABLE_STATIC_TSRMLS_CACHE=1"

	
		APPD_LINK_FLAGS="-Wl,--exclude-libs=ALL -Wl,--gc-sections -Wl,--hash-style=both"
		
		if(PHP_CURL8 == "no") {
			ERROR('This extension requires curl. Use --with-curl8=<PATH> to specify the prefix where curl headers and library are located.');
		}
		ADD_FLAG('CFLAGS', '/I"' + PHP_CURL8 + '\\include"');

		if(PHP_LIBICONV == "no") {
			ERROR('This extension requires libiconv. Use --with-libiconv=<PATH> to specify the prefix where libiconv headers and library are located.');
		}
		ADD_FLAG('CFLAGS', '/I"' + PHP_LIBICONV + '\\include"');


		if(PHP_PHP_SRC == "no") {
			ERROR('This extension requires php-src. Use --with-php-src=<PATH> to specify the prefix where php-src headers and library are located.');
		}

        ADD_FLAG('CFLAGS', '/I"' + PHP_PHP_SRC + '\\ext\\appdynamics_agent"');

		ADD_FLAG('CFLAGS', '/I"' + PHP_PHP_SRC + '\\TSRM"');

		ADD_FLAG('CFLAGS', '/I"' + PHP_PHP_SRC + '"');


		
    	EXTENSION('appdynamics_agent', 'agent.cpp agent_api.cpp agent_logger.cpp agent_thread_globals.cpp backend_identifier.cpp backend_resolver.cpp clientrypoint.cpp configs.cpp correlation.cpp current_exit_call.cpp data_interceptor.cpp delayed_functions.cpp dit_flags.cpp entrypoint.cpp errors.cpp exitpoint.cpp expressions.cpp extension_globals.cpp handle_registry.cpp handle_tracking_exitpoint.cpp infopoints.cpp instrumentation.cpp intercept.cpp method_gatherer.cpp name_value_pair.cpp naming.cpp payload.cpp reporting.cpp request_context.cpp safe_module_shutdown.cpp services.cpp snapshot.cpp snapshot_exitcall.cpp string_match.cpp sensitive_data.cpp sensitive_data_filter.cpp transactions.cpp util.cpp zmqtransport.cpp analytics.cpp analytics_config.cpp analytics_collector.cpp call_graph.cpp call_graph_collection_state.cpp call_graph_node.cpp call_graph_node_name.cpp caching.cpp caching_exitpoint.cpp memcache_exitpoint.cpp memcached_exitpoint.cpp predis_exitpoint.cpp phpredis_exitpoint.cpp database.cpp db_exitpoint.cpp mysql_exitpoint.cpp mysqli_exitpoint.cpp pdo_exitpoint.cpp pgsql_exitpoint.cpp snapshot_sql_info.cpp sqlutil.cpp config.cpp context.cpp main_script_interceptor.cpp codeigniter.cpp detector.cpp drupal7.cpp drupal8.cpp frameworks.cpp laravel.cpp fuelphp.cpp cakephp.cpp mvc.cpp symfony1.cpp symfony.cpp wordpress3.cpp zend1.cpp magento2.cpp zend2.cpp laminas3.cpp curl_exitpoint.cpp drupal7_http_exitpoint.cpp fopen_exitpoint.cpp http.cpp http_correlation.cpp httpentrypoint.cpp httpexitpoint.cpp php_url.cpp zend1_http_exitpoint.cpp zend2_http_exitpoint.cpp amqp_exitpoint.cpp queue.cpp queue_exitpoint.cpp nusoap_exitpoint.cpp native_soapserver_entrypoint.cpp soapclient_exitpoint.cpp webservice.cpp webservices_entrypoint.cpp webservices_exitpoint.cpp BackendCfg.pb.cc Common.pb.cc EPType.pb.cc PHPAgentProtobufs.pb.cc Instrumentation.pb.cc TransactionMatchPointConfig.pb.cc', '$ext_shared=1', '/DZEND_ENABLE_STATIC_TSRMLS_CACHE=1 /std:c++14 /DTIMER_USE_CGT_THREAD /DAGENT_LOGGING_ENABLED');
		
 } 
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\appdynamics_agent2\\config.w32'));
/* ARG_ENABLE("appdynamics_agent", "whether to enable AppDynamics agent support", "yes"); */
/* ARG_ENABLE("appdynamics_asserts", "whether to enable asserts in AppDynamics agent", "no"); */
/* ARG_WITH("apr", "for apr location", "no"); */
/* ARG_WITH("apr-util", "for apr-util location", "no"); */
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\bcmath\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\bz2\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\calendar\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\com_dotnet\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\ctype\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\curl\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\date\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\dba\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\dl_test\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\enchant\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\ffi\\config.w32'));
/* ARG_WITH('ffi', 'ffi support', 'no'); */
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\fileinfo\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\filter\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\ftp\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\gd\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\gettext\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\gmp\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\hash\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\iconv\\config.w32'));
// vim: ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\imap\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\intl\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\json\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\ldap\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\mbstring\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\mysqlnd\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\oci8\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\odbc\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\opcache\\config.w32'));
/* ARG_ENABLE("opcache", "whether to enable Zend OPcache support", "yes"); */
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\openssl\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\pcre\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\pgsql\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\pspell\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\readline\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\reflection\\config.w32'));
// vim:ft=javascript

EXTENSION("reflection", "php_reflection.c", false /* never shared */, "/DZEND_ENABLE_STATIC_TSRMLS_CACHE=1");
PHP_REFLECTION="yes";
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\session\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\shmop\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\snmp\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\sockets\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\sodium\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\spl\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\sqlite3\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\standard\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\sysvshm\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\tidy\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\tokenizer\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\zend_test\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\zip\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\zlib\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\libxml\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\dom\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\exif\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\mysqli\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\pdo\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\pdo_dblib\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\pdo_firebird\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\pdo_mysql\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\pdo_oci\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\pdo_odbc\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\pdo_pgsql\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\pdo_sqlite\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\phar\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\simplexml\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\soap\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\xml\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\xmlreader\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\xmlwriter\\config.w32'));
// vim:ft=javascript
...
configure_module_dirname = condense_path(FSO.GetParentFolderName('C:\\windows-test16\\php-sdk-binary-tools-master\\phpdev\\vc16\\x64\\php-8.1.29-src\\ext\\xsl\\config.w32'));
// vim: ft=javascript
